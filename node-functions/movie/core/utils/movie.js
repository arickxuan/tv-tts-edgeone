...
export function getImageProxyUrl(){
  ...
  if (enableImageProxy !== null) {
    if (!JSON.parse(enableImageProxy) ) {
      return null;
    }
  }
  ...
  // 如果未设置，则使用全局对象
  const serverImageProxy = (window ).RUNTIME_CONFIG?.IMAGE_PROXY;
...
export function getDoubanProxyUrl() {
  ...
  // 如果未设置，则使用全局对象
  const serverDoubanProxy = (window ).RUNTIME_CONFIG?.DOUBAN_PROXY;
 ...
 */
export function processDoubanUrl(originalUrl) {
  if (!originalUrl) return originalUrl;
 ...
 */
export async function getVideoResolutionFromM3u8(m3u8Url){
  try {
