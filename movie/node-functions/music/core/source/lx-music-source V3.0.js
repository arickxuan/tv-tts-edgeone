...
const cuFetch = (url, options = { method: 'GET' }) => {
  return new Promise((resolve, reject) => {
    console.log('--- start --- ' + url)
    axios(url, options).then(resp => {
      console.log('API Response: ', resp)
      resolve(resp)
    }).catch(err => {
...
const handleGetMusicUrl = async (source, musicInfo, quality) => {
  if (source == 'local') {
    ...
    const request = await cuFetch(targetUrl, {
      method: 'GET',
      headers: {
  ...
  const request = await cuFetch(`${API_URL}/lxmusicv3/url/${source}/${songId}/${quality}`, {
    method: 'GET',
    headers: {
      'Content-Type': 'application/json',
      'User-Agent': agent,
  ...
  switch (source) {
    case 'local':
      ...
      if (body.code === 0 && body.data.lyric) {
        var t = 'l'
        var b = handleBase64Encode(JSON.stringify(requestBody))/* url safe*/.replace(/\+/g, '-').replace(/\//g, '_')
        const request2 = await cuFetch(`${API_URL}/local/${t}?q=${b}`, {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
            'User-Agent': agent
...
const checkUpdate = async () => {
  const request = await cuFetch(`${API_URL}/script?key=${API_KEY}&checkUpdate=${SCRIPT_MD5}`, {
    method: 'GET',
    headers: {
      'Content-Type': 'application/json',
      'User-Agent': agent
