...
async function searchMusic(query, page) {
    ...
    const result = (await (0, axios)(config)).data.data;
    return {
...
async function getTopListDetail(topListItem) {
    if (topListItem.type === "playlist") {
        const res = (await axios.get(`https://www.udio.com/api/playlists?id=${topListItem.id}`, {
            headers,
        })).data;
        const songList = res.playlists[0].song_list.join(",");
        const songs = (await axios.get(`https://www.udio.com/api/songs?songIds=${songList}`, {
            headers,
        })).data.songs;
        return {
            isEnd: true,
        ...
        const songs = (await (0, axios)(config)).data.data;
        return {
            isEnd: true,
            musicList: songs.map(formatMusicItem),
        };
